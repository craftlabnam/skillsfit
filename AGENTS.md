...
- **EXTRA IMPORTANT**: This project uses Tailwind CSS v4 (CSS-first `@theme` tokens in `app/globals.css`, no `tailwind.config.ts`) — see `context/ui-tokens.md`. This supersedes InsForge's generic boilerplate guidance to lock to 3.4; do not downgrade.
