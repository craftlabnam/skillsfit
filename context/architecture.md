...
| Cloud browser                  | Firecrawl                | Company research — browsing company public pages |
| AI browser control             | Firecrawl                | Company page interaction and content extraction  |
...
/
├── AGENTS.md
├── proxy.ts                                 → Session refresh + protected-route gate (Next.js 16+ name for middleware.ts)
├── context/
│   ├── project-overview.md
│   ├── architecture.md
│   ├── ui-tokens.md
...
│       │   └── callback/route.ts          → OAuth callback — exchanges code for session, sets cookies, redirects
│       ├── agent/
...
│   ├── research.ts                        → Company research — Firecrawl + Stagehand + GPT-4o
...
├── actions/
│   ├── auth.ts                            → signInWithPassword, signInWithOAuth, signOut Server Actions
...
│   ├── insforge-server.ts                 → InsForge server client (read-only)
│   ├── auth-cookies.ts                    → Shared OAUTH_CODE_VERIFIER_COOKIE constant
│   ├── auth-redirect.ts                   → getPostSignInRedirect() — hardcoded to /dashboard until Feature 04/06 add profiles.is_complete
│   ├── firecrawl.ts                       → Firecrawl session creation + management
│   ├── stagehand.ts                       → Stagehand initialisation with Firebase session
...
Calls agent/research.ts
        ↓
Single Firecrawl session opens with Stagehand
        ↓
...
- Provider: InsForge Auth, SSR cookie mode (`@insforge/sdk/ssr`)
- Methods: Google OAuth, GitHub OAuth
- Protected routes: /dashboard, /profile, /find-jobs, /find-jobs/[id]
- Public routes: /, /login
- Session lives in cookies, set server-side only: `insforge_access_token` (readable, short-lived) + `insforge_refresh_token` (httpOnly). Never write these manually — only auth actions and `updateSession()` touch them.
- **`proxy.ts`, not `middleware.ts`** — Next.js 16+ (this project's version) renamed the middleware convention. The file exports `proxy()` instead of `middleware()`. It calls `updateSession()` on every request to refresh the access token, then checks it against the protected-route list and redirects to `/login` when absent.
- OAuth is server-driven, not client auto-detected — see "InsForge Client Pattern" below.
...
Package is `@insforge/sdk` (verified against npm — `@insforge/ssr` is not a real package). SSR helpers ship as subpath exports of the same package. Three separate surfaces — never mix them:

```typescript
// lib/insforge-client.ts
// Browser-side — used in Client Components for reads via Storage/Realtime
...
// Server-side reads — used in Server Components, Route Handlers, Server Actions, agent code
import { createServerClient } from "@insforge/sdk/ssr";
import { cookies } from "next/headers";

export const createInsforgeServer = async () => {
  return createServerClient({
    cookies: await cookies(), // read-only — cannot sign in/out
  });
};
```

```typescript
// Auth mutations only (sign in/up/out, OAuth) go through createAuthActions(),
// never through createInsforgeServer() above — that client is read-only.
"use server";
import { cookies } from "next/headers";
import { createAuthActions } from "@insforge/sdk/ssr";

const auth = createAuthActions({ cookies: await cookies() });
// auth.signInWithOAuth(...), auth.signOut(), etc. — sets/clears the session cookies for you
```

```typescript
// proxy.ts — root level, Next.js 16+ naming (see Authentication section above)
...
- Browser client (`lib/insforge-client.ts`) — Client Components, browser-side reads only
- Server client (`lib/insforge-server.ts`) — Server Components, API routes, Server Actions, agent functions; read-only, cannot sign in/out
- Auth actions (`createAuthActions()`) — the only surface that mutates the session cookie; used in Server Actions and the OAuth callback Route Handler
- `proxy.ts` — session refresh + protected-route gate, replaces `middleware.ts` on Next.js 16+
- Never use the browser client in server context, and never use the server client in browser context

---

## Firecrawl Session Pattern
...
- Firecrawl sessions are always closed with stagehand.close() when done — never leave sessions open.
