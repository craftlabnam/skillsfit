...
InsForge authentication — email/password and Google OAuth. (Originally scoped as Google + GitHub OAuth only; revised by direct request to drop GitHub and add email/password.)

**UI:**

- Two-column login page — left panel: heading "Sign in and let the agent prep your next application" + supporting copy; right panel: card titled "Welcome" with email/password form above a Google OAuth button (GitHub button removed)

**Logic:**

- Email/password sign-in via InsForge (sign-in only — no sign-up form)
- Google OAuth via InsForge (`signInWithOAuth(provider)` still accepts `"github"` in code, just unused by any button)
- OAuth callback handler
- Session management
- Middleware (`proxy.ts` — Next.js 16+ naming) protecting /dashboard, /profile, /find-jobs, /find-jobs/[id]
- After sign-in → everyone redirects to /dashboard for now (new-vs-existing-user routing to /profile for onboarding is deferred until Feature 04/06 add a real `profiles.is_complete` signal)
...
- Create instrumentation-client.ts — PostHog browser client, initialized with NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN and NEXT_PUBLIC_POSTHOG_HOST (Next.js 15.3+/16 pattern — replaces the originally planned lib/posthog-client.ts + root-layout provider, which this Next.js version deprecates in favor of an auto-loaded instrumentation file)
- Reverse proxy in next.config — /ingest/* rewritten to PostHog's ingestion + asset hosts, so the browser client's api_host is same-origin
- Create lib/posthog-server.ts — PostHog server client (`getPostHogClient()` singleton) with flushAt: 1 and flushInterval: 0; every server-side call site awaits `posthog.flush()` after capturing
- posthog.identify() called after successful login with user ID (both password and OAuth sign-in)
- posthog.reset() called on logout, client-side in SignOutButton.tsx after signOut() succeeds
...
Agent researches the company using their public website and builds a structured dossier using a single Firecrawl session. Three data sources fused together: company website content, job description from DB, user profile from DB.
...
- Open single Firecrawl session with Stagehand
...
- Close Firecrawl session after homepage + max 3 sub-pages
